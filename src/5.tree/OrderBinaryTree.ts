...
export class OrderBinaryTree<T extends number | Comparator<T>> {
  ...
  preOrder(cb: (node: Node<T>) => boolean) {
    let stack = [this.root];
  ...
  preOrder1(cb: (node: Node<T>) => boolean) {
    let node = this.root;
    let stack: Node<T>[] = [];
    while (true) {
      if (node) {
  ...
  inOrder(cb: (node: Node<T>) => boolean) {
    let node = this.root as Node<T>;
    let stack: Node<T>[] = [];
  ...
  postOrder(cb: (node: Node<T>) => boolean) {
    let node = this.root as Node<T>;
    let stack: Node<T>[] = [node];
